...
async fn test_recording_pcap_integrity() {
    ...
    while let Some(pkt) = reader.next_packet() {
        ...
        if data.len() > payload_offset {
            ...
            // Codec descriptor packets (RBP1 + JSON) lead each stream; skip them.
            if !payload.starts_with(b"RBP1") {
                assert_eq!(payload[0] & 0xC0, 0x80, "RTP version should be 2");
            }
        }
...
async fn test_recording_pcap_contains_rtcp() {
    ...
    // Start session-wide recording. Recording is inbound-only — outbound RTP/RTCP
    // that rtpbridge sends to peers is intentionally not captured.
    ...
    // Outbound RTCP (rtpbridge → peer) is no longer recorded, and this test peer
    // sends no inbound RTCP, so none is expected in the capture.
    assert_eq!(
        rtcp_count, 0,
        "no inbound RTCP is sent by the test peer (got {rtcp_count} RTCP, {rtp_count} RTP)"
    );
