...
async fn test_rtcp_recorded_with_srtp_session() {
    ...
    // Start session-wide recording. RTCP SR/RR is generated by rtpbridge and sent
    // outbound to peers, so this test must opt into outbound capture or no RTCP
    // will land in the PCAP.
