...
async fn test_rtcp_recorded_with_srtp_session() {
    ...
    // Start session-wide recording. Recording is inbound-only — it captures the
    // decrypted SRTP each peer sends; outbound RTP/RTCP that rtpbridge sends to
    // peers is intentionally not captured.
    ...
    let rec = client
        .request_ok(
            ...
            json!({
                "endpoint_id": null,
                "file_path": pcap_path.to_str().unwrap(),
            }),
        )
        .await;
    ...
    while let Some(pkt) = reader.next_packet() {
        ...
        // The recorder captures decrypted inbound SRTP as plaintext RTP. Inbound
        // RTCP (PT 200=SR, 201=RR) is captured too when a peer sends it; these
        // test peers don't, so we only assert on the RTP that peer A sends.
        if pt == 200 || pt == 201 {
            rtcp_count += 1;
        } else {
            rtp_count += 1;
    ...
    // Outbound RTCP (rtpbridge → peer) is no longer recorded, and the test peers
    // send no inbound RTCP, so none is expected here.
    assert_eq!(
        rtcp_count, 0,
        "no inbound RTCP is sent by the test peers (got {rtcp_count})"
    );
